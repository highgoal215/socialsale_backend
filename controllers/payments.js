...
const checkoutService = require('../utils/checkoutService'); // New service for Checkout.com
const ErrorResponse = require('../utils/errorResponse');
const { createNotification } = require('./notifications');

exports.getPaymentMethods = async (req, res, next) => {
  try {
    // Simplified to just show Checkout.com as the payment option
    const paymentMethods = [
      {
        id: 'checkout',
        name: 'Credit/Debit Card',
        description: 'Pay with Visa, Mastercard, or American Express via Checkout.com',
        active: true,
        fees: '2.9% + $0.30',
        icon: 'credit-card'
      }
      // You can add more payment methods that Checkout.com supports
    ];
...
exports.checkoutWebhook = async (req, res, next) => {
  try {
    // Process webhook from Checkout.com
      ...
      if (transaction.orderId) {
        ...
        if (order) {
          await createNotification(
            ...
            'payment',
            'Payment Failed',
            `Your payment of $${transaction.amount} has failed. Please try again or contact support.`,
            {
              link: `/orders/${order._id}`,
              relatedId: order._id,
              onModel: 'Order'
