...
exports.createOrder = async (req, res, next) => {
  try {
    ...
    const {
      socialUsername,
      serviceType,
      serviceQuality, // This is what user sends, but model expects 'quality'
      serviceId,      // Optional - user can provide this directly or we find by type/quality
      quantity,
      postUrl,
      deliverySpeed
    } = req.body;
    ...
    if (serviceId) {
      ...
      // Map serviceQuality to quality (if they use different terms)
      const quality = serviceQuality === 'premium' ? 'premium' : 'general';
    ...
    // Create order data with ALL required fields from the model
    const orderData = {
      userId: req.user.id,
      serviceId: service._id,                      // Required by your model
      supplierServiceId: service.supplierServiceId, // Required by your model
      socialUsername,
      serviceType,
      quality: service.quality,                    // Required by your model
      quantity,
      price: service.price,
      supplierPrice: service.supplierPrice,        // Required by your model
      postUrl,
      postId,
      paymentMethod: 'checkout',                   // Set default to checkout.com
    ...
    await createNotification(
      ...
      'order_update',
      'New Order Created',
      `Your order for ${order.quantity} ${order.serviceType} has been created successfully.`,
      {
    ...
    await createAdminNotification(
      'order_update',
      'New Order Placed',
      `A new order has been placed by user ${order.socialUsername} for ${order.quantity} ${order.serviceType}.`,
      {
        link: `/orders/${order._id}`,
        relatedId: order._id,
        onModel: 'Order'
...
exports.processOrder = async (req, res, next) => {
  try {
    ...
    await createNotification(
      ...
      `Your order for ${order.quantity} ${order.serviceType} is now being processed.`,
      {
        link: `/orders/${order._id}`,
        relatedId: order._id,
        onModel: 'Order'
    ...
    // Admin notification for processing started
    await createAdminNotification(
      'order_update',
      'Order Processing Started',
      `Order for user ${order.socialUsername} (${order.quantity} ${order.serviceType}) is now being processed.`,
      {
        link: `/orders/${order._id}`,
        relatedId: order._id,
        onModel: 'Order'
    ...
    try {
      ...
      // Create notification for successful order processing
      await createNotification(
        order.userId,
        'order_update',
        'Order Sent to Supplier',
        `Your order has been successfully sent to our supplier and is being processed.`,
        {
          link: `/orders/${order._id}`,
          relatedId: order._id,
          onModel: 'Order'
      ...
      // Admin notification for sent to supplier
      await createAdminNotification(
        'order_update',
        'Order Sent to Supplier',
        `Order for user ${order.socialUsername} (${order.quantity} ${order.serviceType}) has been sent to supplier.`,
        {
          link: `/orders/${order._id}`,
          relatedId: order._id,
          onModel: 'Order'
      ...
      await createNotification(
        ...
        `Your order processing failed. Please contact support for assistance.`,
        {
          link: `/orders/${order._id}`,
          relatedId: order._id,
          onModel: 'Order'
      ...
      // Admin notification for failed order
      await createAdminNotification(
        'order_update',
        'Order Processing Failed',
        `Order for user ${order.socialUsername} (${order.quantity} ${order.serviceType}) failed to process.`,
        {
          link: `/orders/${order._id}`,
          relatedId: order._id,
          onModel: 'Order'
...
// @desc    Handle supplier webhook for order status updates
// @route   POST /api/orders/supplier-webhook
// @access  Public (supplier webhook)
exports.handleSupplierWebhook = async (req, res, next) => {
  try {
    const { order_id, status, start_count, remains } = req.body;
    
    if (!order_id || !status) {
      return res.status(400).json({ success: false, message: 'Missing required fields' });
    }
    
    // Find order by supplier order ID
    const order = await Order.findOne({ supplierOrderId: order_id });
    ...
    // Update order info
    order.status = newStatus;
    order.startCount = start_count || 0;
    order.remains = remains || 0;
    await order.save();
...
exports.updateOrderStatus = async (req, res, next) => {
  try {
    ...
    // Validate status
    const validStatuses = ['pending', 'processing', 'completed', 'partial', 'canceled', 'failed'];
